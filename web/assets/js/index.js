...
var currentPath;
	...
	function countPercent(){
        ...
        // alert("countSizeNum"+countSizeNum);
...
// 例如，3.5 将舍入为 4，而 -3.5 将舍入为 -3。
		...
		// alert("countSizeNum * 100"+countSizeNum * 100);
		// alert(countSizeNum * 100 / totalSizeNum);
		// alert(percent);
		// $("#sizeprogress").css("width", percent).attr("aria-valuemax", totalSizeNum).text(percent);
        $("#sizeprogress").html(percent);
        // alert( $("#sizeprogress").html(percent).text());
		//alert(totalSizeNum);
		//alert(countSizeNum);
		// $("#progressview").attr("aria-valuemax",totalSizeNum/(1024*1024));
       //不足，如果大于100则崩溃
        var percentageValue =Math.round(countSizeNum *100/ totalSizeNum);
        alert("比例值"+percentageValue);
		$("#progressview").attr("class","progress-bar progress-bar-primary progress-animated wow animated progress-"+33);
	}
	/**获取文件列表*/
	function getFiles(path) {
		alert("传回的地址"+path);
		$.post("files/getFiles.action", {
			"path" : path
		}, function(data) {
			if (data.success) {
				currentPath = path;
	...
	function navPath(path, currentPath) {
		$("#navPath").append(
				'<a href="#" path="' + path
						...
						+ currentPath + '<i class="icon-chevron-sign-right" style="color: #59555F"></i></a>');
	}
	/**上传文件点击事件*/
	function upload(obj) {
		$("#input_file").click();
	...
	*/
		...
		if ($check.length > 1 || $check.length <= 0) {
			...
			layer.prompt({title: '重命名', value : srcName}, function(destName, index){
				  if(destName.trim() == ""){
					  ...
					  $.post("files/renameDirectory.action",{
	...
	///**新建文件夹 */
 //	function buildfile(){
        // bs4pop.prompt('文件名:', 'Input Placeholder', function(filename, index){
        ...
        //             	alert("daoda");
        //                         $.post("files/addDirectory.action",{
        //                             "currentPath":currentPath,
	...
	function copyto(){
		...
		if($id.length<1){
			...
			layer.open({
				...
				content: 'files/summarylist.action',
				btn: ['确定', '取消'],
				yes: function(index,layero){
					...
					if(cancopy == "yes"){
						$.ajax({
							type : "POST",
							url : "files/copyDirectory.action",
							data : {
								"currentPath" : currentPath,
								"directoryName" : check,
...
// 					currentPath = path;
	...
	function joinUrl(url){
        ...
        alert("qian"+host)	;
		host = host.substring(0, host.indexOf("login"));
		alert(host)	;
		return host + "files/shareFiles.action?shareUrl=" + url;
	}
	/**打开我的分享*/
			...
			if (data.success) {
				...
				if(type == "image"){
					...
					$.each(data.data, function() {
						...
						$("#"+ typeName).append('<a href="files/openFile.action?'+url+'" data-lightbox="roadtrip" title="'+this.fileName+'"><img alt="'+this.fileName+'" style="margin: 10px" src="files/openFile.action?'+url+'" width="150" height="150"></a>')
					});
				}else{
	...
	function openFile(obj) {
		...
		if (fileType == "folder-open") {
            ...
            $(obj).attr({"href":"files/openFile.action?"+url,"data-lightbox":"test","data-title":fileName});
            return true;

		} else if(fileType.indexOf("office") >= 0){
			$.post("files/openOffice.action", {
			...
			layer.open({
				type:2,
				title:'播放',
				content:'files/openAudioPage.action?' + url,
				shade: [0],
				area: ['440px', '120px'],
				offset: 'rb', //右下角弹出
			});
		} else if(fileType.indexOf("docum") >= 0){
			$.post("files/openFile.action", {
				"currentPath" : parentPath,
				"fileType" : fileType,
				"fileName" : fileName,
			}, function(data){
